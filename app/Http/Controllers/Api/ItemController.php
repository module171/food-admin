...
class ItemController extends Controller
{
    public function item(Request $request)
    {
        ...
        if($request->user_id == ""){
            ...
            if(!empty($itemdata))
            {
                return response()->json(['status'=>1,'message'=>'Item Successful','data'=>$itemdata,'currency'=>env('CURRENCY'),'max_order_qty'=>$getdata->max_order_qty,'min_order_amount'=>$getdata->min_order_amount,'max_order_amount'=>$getdata->max_order_amount],200);
            }
            else
            {
